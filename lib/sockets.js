if (Meteor.isClient) {
  Socket = {
    ...
    transactions: function () {
      ...
      websocket.onmessage = function (e) {
        if (!_.find($('.bubble'), function (circle) { return circle.style.opacity != 0; } )) {
          $('.bubble').remove();
        }
        ...
        if (data.x) {
          ...
          d3.select("#bubbles").append('circle')
            ...
            .style("opacity", 0)
            .each("end", function () {
              if (!_.find($('circle'), function (circle) { return circle.style.opacity != 0; } )) {
                d3.select(".combo")
                .text($('.combo').text())
                .transition()
                .duration((1000 / combo) * 4)
                .ease('linear')
                .tween("text", function() {
                  var i = d3.interpolate(this.textContent, 0);
                  return function(t) {
                    this.textContent = "combo " + Math.round(i(t));
                  };
                });
              }
            });
        ...
        d3.select(".combo")
          ...
          .ease('linear')
          .tween("text", function() {
            var i = d3.interpolate(this.textContent, combo);
            return function(t) {
              this.textContent = "combo " + Math.round(i(t));
            };
          });

        if (combo > (Session.get("high") || 0)) {
          d3.select(".high").text("high " + combo);          
          Session.set("high", combo);
        }
